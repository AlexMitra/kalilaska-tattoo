...
public class UnapprovedMasterEventNoticeTag extends AbstractI18nMessageTag{
	private final static Logger LOGGER = LogManager.getLogger(UnapprovedMasterEventNoticeTag.class);
    ...
    public int doStartTag() throws JspTagException {
		...
		if(consultations != null) {
			...
			while (count < consultations.size()) {
				...
				if(consultation.isApproved() == false) {
	...
	private void makeNotice(String message) {
		if(message != null) {
			try {				
				...
				out.write("<span class='signal-message working-message'>");
				out.write(message);
				out.write("</span>");
			} catch (IOException e) {
