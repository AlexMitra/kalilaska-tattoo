...
public class ForbideAccountCommand extends AbstractPrgCommand {
	
	private AccountService accountService;

	public ForbideAccountCommand(AccountService accountService, String redirectedURI) {
		super(redirectedURI);
    ...
    protected void handle(SessionRequestContent content) {
    	...
    	if(bean.getClass().equals(AdminPersonalAreaViewBean.class)) {
        	String[] idArr = content.getRequestParameters().get(RequestParamNameList.PARAMETER_FOR_FORBIDE_ACCOUNT);
        	
        	if(idArr != null && idArr.length > 0) {
        		String accountId = idArr[0];            		
        		boolean forbidden = accountService.forbideAccountById(Integer.valueOf(accountId));
        		if(forbidden) {
        			...
        			content.insertSessionAttribute(SessionAttrNameList.ATTRIBUTE_FOR_FORBIDE_ACCOUNT_FAILURE, 
        					WebMessageManager.getMessage(MessageNameList.FORBIDE_ACCOUNT_ERROR));
        		}
        	}else {
    			content.insertSessionAttribute(SessionAttrNameList.ATTRIBUTE_FOR_FORBIDE_ACCOUNT_FAILURE, 
