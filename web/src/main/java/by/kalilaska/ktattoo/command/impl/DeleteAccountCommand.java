...
public class DeleteAccountCommand extends PersonalAreaViewCommand {

	private AccountService accountService;

	public DeleteAccountCommand(AccountService accountService, String viewPath, String viewBodyPath, 
    ...
    protected void setContent(SessionRequestContent content) {
    	...
    	if(bean.getClass().equals(AdminPersonalAreaViewBean.class)) {
        	String[] paramArr = content.getRequestParameters().get(RequestParamNameList.PARAMETER_FOR_DELETE_ACCOUNT);
        	
        	if(paramArr != null && paramArr.length > 0) {
        		String accountId = paramArr[0];            		
        		boolean flag = accountService.deleteAccountById(Integer.valueOf(accountId));
        		if(flag) {
        			...
        			makeWrongMessage(content, SessionAttrNameList.ATTRIBUTE_FOR_DELETE_ACCOUNT_FAILURE, 
        					MessageNameList.DELETE_ACCOUNT_ERROR);
        		}
        	}else {
        		makeWrongMessage(content, SessionAttrNameList.ATTRIBUTE_FOR_DELETE_ACCOUNT_FAILURE, 
